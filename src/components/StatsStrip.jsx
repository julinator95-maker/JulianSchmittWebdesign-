...
function StatItem({ prefix, value, suffix, label, inView, delay, reduce }) {
  ...
  return (
    ...
    >
      <div className="text-6xl md:text-7xl font-extralight tracking-tight text-ink mb-3 tabular-nums">
        {prefix}<span className="text-accent">{count}</span>{suffix}
      </div>
      <p className="text-xs font-light tracking-[0.12em] text-muted uppercase leading-relaxed">
        {label}
      </p>
    </motion.div>
  )
...
export default function StatsStrip() {
  ...
  return (
    <section ref={ref} className="bg-stone py-16 md:py-20 px-6 md:px-12 border-b border-border">
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-10 sm:gap-0 sm:divide-x divide-border">
